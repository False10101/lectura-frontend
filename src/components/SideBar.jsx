  ...
  useEffect(() => {
    try {
      const fetchHistory = async () => {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/note/history`, {
          method: 'GET',
          headers: {
    ...
    <div className="w-[16vw] h-full border-rose-950 px-3  flex flex-col">
      <div className="w-full h-min flex py-4 border-b-[1px] border-[#6B7280]/30 items-center">
        ...
        <div className="flex flex-col justify-center">
          <span className="text-2xl text-[#4C1D95] font-bold ml-3 -mt-1">Lectura</span>
      ...
      <div className="w-full h-min flex py-4 flex-col flex-grow overflow-hidden pb-9">
        <div onClick={()=> handleRedirect("/note")} className={`w-[90%] flex h-max py-3 ${pathname === "/note" || pathname === "" ? "bg-[#4C1D95] drop-shadow-lg text-white " : ""} rounded-xl px-3 items-center`}>
          <HomeIcon className="w-4 h-4 " />
          <span className="truncate ml-2">Home</span>
        </div>
        <div onClick={()=> handleRedirect("/quiz")} className={`w-[90%] flex h-max py-3 ${pathname === "quiz" ? "bg-[#4C1D95] drop-shadow-lg text-white " : ""} rounded-xl px-3 items-center`}>
          ...
          <div className="w-full flex flex-col overflow-auto">
            {
              historyList.map((history, index)=>(
                <div key={index} className="w-full h-min py-0.5 px-2 my-3 flex">
                  ...
                  <div className="truncate w-[90%] ml-3">{history.name}</div>
                </div>
              ))
            }
          </div>
